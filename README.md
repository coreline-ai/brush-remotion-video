...
[![Tests](https://img.shields.io/badge/tests-pytest_312_%C2%B7_vitest_47-2EA44F?logo=githubactions&logoColor=white)](#-검증)
[![Skills](https://img.shields.io/badge/skills-9_installed-FF8C00)](#-스킬-카탈로그)
...
- **TTS 음성팩**: 여성 10종 조회·청취·검증 `python3 bin/voice-assets.py validate` — [선택표](skill/_shared/references/supertonic-voice-catalog.md)
...
python3 bin/bgm-assets.py gate       # 10곡 + 필수 E2E 4종 + 라이선스 + 사람 승인 최종 판정
...
├── skill/                   # catalog + 공통 계약 + 얇은 스킬 9종 (코드 사본 0)
├── examples/                # 기존 모드 + 로컬 BGM project.yaml 예제
├── tests/                   # vitest 47건 + golden/ 픽셀 diff 게이트
...
```bash
npm run check-schema && npm run typecheck && npm test  # schema sync + tsc + vitest 47건
