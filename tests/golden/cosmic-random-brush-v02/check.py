#!/usr/bin/env python3
"""Validate the six-scene cosmic-random-brush v0.2 matrix."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--routes-dir", type=Path,
        default=ROOT / "public/cosmic-random-brush-v02/routes")
    parser.add_argument(
        "--qa-report", type=Path,
        default=ROOT / "data/cosmic-random-brush-v02/qa/cosmic-random-brush-report.json")
    ...
    for item in matrix["scenes"]:
        ...
        if not source.is_file() or sha256(source) != item["normalizedSha256"]:
            failures.append(f"scene-{item['scene']:02d}: source hash mismatch")
            continue
        route_path = args.routes_dir / f"scene-{item['scene']:02d}.routes.json"
        if not route_path.is_file():
            failures.append(f"scene-{item['scene']:02d}: routes missing")
            continue
        ...
        checks = {
            ...
            "brushWidth": meta.get("brushWidthRange", [0, 9999])[0]
            >= quality["brushWidthRange"][0]
            and meta.get("brushWidthRange", [0, 9999])[1]
            <= quality["brushWidthRange"][1],
            "maskCoverage": float(meta.get("maskCoverage", 0))
            >= quality["maskCoverageMin"],
            "visibleContentCoverage": float(meta.get("visibleContentCoverage", 0))
            >= quality["visibleContentCoverageMin"],
            "meanJump": float(meta.get("meanCenterJump", 0))
            >= quality["meanCenterJumpMin"],
            "maxJump": float(meta.get("maxCenterJump", 0))
            >= quality["maxCenterJumpMin"],
            "drawEnd": float(meta.get("drawEnd", 999)) <= quality["drawEndMax"],
        }
        for name, ok in checks.items():
            if not ok:
                failures.append(f"scene-{item['scene']:02d}: {name}")
        print(
            f"{'PASS' if all(checks.values()) else 'FAIL'} scene-{item['scene']:02d} "
            f"mask={meta.get('maskCoverage')} content={meta.get('visibleContentCoverage')} "
            f"strokes={meta.get('strokeCount')} width={meta.get('brushWidthRange')}"
        )
        if item["scene"] == 1:
    ...
    if not args.qa_report.is_file():
        ...
        if report.get("pass") is not True or len(report.get("scenes", [])) != 6:
            failures.append("profile QA report did not pass six scenes")
