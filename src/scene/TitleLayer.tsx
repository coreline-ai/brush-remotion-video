...
export const TitleLayer: React.FC<{ frame: number; spec: TopTitle }> = ({ frame, spec }) => {
  ...
  const fontSize = spec.fontSize ?? 60;
  const kickerFontSize = spec.kickerFontSize ?? 20;
  ...
  return (
    <div
      style={{
        ...
        padding: spec.wash ? "8px 12px 10px" : 0,
        borderRadius: 18,
    ...
    >
      {spec.kicker && (
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16 }}>
          {spec.align === "center" ? (
            <>
              <span style={{ flex: 1, minWidth: 70, height: 2, background: accent, opacity: 0.48 }} />
              {kickerText}
              <span style={{ flex: 1, minWidth: 70, height: 2, background: accent, opacity: 0.48 }} />
            </>
          ) : spec.align === "left" ? (
            <>
              <span style={{ width: 36, height: 2, background: accent, opacity: 0.82 }} />
              {kickerText}
              <span style={{ flex: 1, minWidth: 34, height: 2, background: accent, opacity: 0.35 }} />
            </>
          ) : (
            <>
              <span style={{ flex: 1, minWidth: 34, height: 2, background: accent, opacity: 0.35 }} />
