...
## ✒️ pen 프로파일용 — 선화(line-art) 배경 변형

`drawing.profile: pen`일 때는 수채 워시 대신 **펜 선화 스케치**를 생성한다.
잉크-알파 분리(파이프라인 자동)가 전제이므로 배경은 선이 또렷하고 종이가 균일할수록 좋다.

```
A clean hand-drawn PEN sketch illustration (fine black ink line-art), like a whiteboard
explainer drawing. Subject: {SUBJECT}. Crisp confident pen outlines, selective accent
colors ONLY in blue (#2b6bf0) and orange (#f07b2b) for emphasis lines, NO watercolor washes,
NO shading gradients, uniform warm-white paper background. Airy composition with generous
empty space. Absolutely NO text, no letters, no numbers. Minimal, precise, 16:9 landscape.
```
