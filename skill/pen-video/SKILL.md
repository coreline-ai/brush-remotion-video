...
`bgm` 블록이 없는 15~120초 무음 pen 영상은 Stable Audio 피아노 후보를 자동 우선 생성한다. 음성/TTS 영상에서 생성 BGM이 필요하면 `bgm.mode: piano-auto`를 명시한다.
...
| 씬 프리셋 | faint 1.0(즉시 또렷) · edgeFeather 1 · developFrames 8(무팝업) · 첫 씬 흐린 포스터 외 prewash 0 · jitter 0 · 순차 드로잉 |
| 커서 | 내장 벡터 펜 스프라이트 (`brush: {kind: "pen", w: 140}`) — 펜촉이 그리는 지점에 닿음 |

## 첫 씬 카드 썸네일 — 흐린 원본 포스터

기본 `pen` 프로파일의 첫 씬은 실제 재생 스트림(`v:0`) 0프레임에 정규화 원본
`scene-01.png`을 `opacity: 0.35`, `blur: 10px`로 표시한다. 12프레임(30fps 기준 0.4초) 동안
종이로 선형 페이드하고, 페이드가 끝나는 f12에 펜 선화를 시작한다. 따라서 카드에는 장면이
보이지만 재생에서는 완성 선화·색면이 먼저 드러나지 않는다.

- 이 포스터는 첫 씬에만 적용하며, 나머지 씬은 종이 → 펜 선화 흐름을 유지한다.
- `preserveSource: true`는 원본 32% 가이드로 색면을 복원하는 별도 연출이므로 이 흐린 포스터를
  중첩하지 않는다.
- MP4 `attached_pic`만으로는 카드 표시를 보장할 수 없으므로 실제 `v:0` 0프레임을 검수한다.
